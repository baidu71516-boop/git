...
class CampaignOutreachAccess:
    ...
    @staticmethod
    def own_department_id(context: AuthContext) -> UUID:
    ...
    @staticmethod
    def require_mutation(context: AuthContext) -> UUID:
    ...
    async def resolve_read_scope(
        ...
        if target_department_id != own_department_id and context.role is not Role.SUPER_ADMIN:
