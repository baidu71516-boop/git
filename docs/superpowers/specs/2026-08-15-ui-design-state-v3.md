# 2026-08-15 UI Design State v3 Checkpoint

此文档为下一轮 UI 设计会话的唯一 checkpoint，基于当前仓库真实实现与 UI-1～UI-6 已冻结内容，供后续会话直接接续。此文档仅描述已冻结界面状态，不复盘实现过程。
...
- 当前禁止：
  ...
  - 本文件为开发文档快照，不执行产品代码、测试、Backend/API/DB 变更
...
## 12. UI-6 Freeze（采集任务筛选规则配置）

- 入口与导航边界：
  - 不新增 Sidebar / route / 顶层 Tab
  - 入口位于“数据采集 → 当前采集任务”
- 入口状态：
  - 有编辑权限：显示“编辑筛选规则”
  - Viewer：显示“查看筛选规则”
- 当前采集任务摘要：
  - 平台
  - 来源标签数量
  - 粉丝范围（`follower_min` / `follower_max`）
  - 规则版本
- 平台：
  - 当前真实平台仅“`小红书`”
- 来源标签：
  - 基于 `source_tags_exact_any`，精确 ANY 匹配（标签原值）
- 编辑弹窗：
  - 有编辑权限可编辑；Viewer 只读展示（避免禁用控件表单风格）
  - 规则版本以副信息展示，不支持编辑
- 保存与冲突：
- 保存调用真实 `PUT screening-rules` endpoint（当前产品接口）
  - 必须携带 `expected_revision`
  - 不自动处理 409 重试，不做 destructive 覆盖
  - 409 提示文案固定：`筛选规则已被其他人更新。请重新加载最新规则后再继续编辑。`
  - 操作按钮：`重新加载最新规则`
- 保存提示语义：
  - `preview_ready` 下提示：`保存后，当前数据预览将失效，需要重新生成。`
  - 保存成功后只做 `refetch`，不得自动 rebuild
  - 不自动 Confirm
- 其他语义：
  - `preview_stale` 继续复用 UI-4 已冻结展示
- Development preview（/dev-ui-preview/data-collection）要求：
  - 场景列表必须包含：
    - 筛选规则 · 可编辑
    - 筛选规则 · 已有数据预览
    - 筛选规则 · 规则冲突
    - 筛选规则 · 只读
  - 预览场景选择器使用完整中文下拉（单选），无横向滚动条
...
## 16. Hallmark-adapted 15 条审核原则（精简）
...
- 冻结范围：UI-1～UI-6 全部 Frozen。
- 除非出现新的 Backend commit 或明确的 contract 冲突，UI-1～UI-6 不重复审计。

## 19. Development Preview Harness
...
### 21.2 最近 UI-5 / UI-6 相关 commits（ui-redesign）
...
### 21.3 UI-6 关键提交（当前检出分支）

- UI-6 产品提交：
  - `84b5d2c` feat(web): add collection screening rule editor
- UI-6 Visual Polish（如存在）：
  - `79699d9` style(web): polish collection screening rule editor
- UI-6 Preview Harness（如存在）：
  - `81d5461` chore(web): improve data collection preview state selector

### 21.4 当前 Backend Task 9 baseline / merge（backend worktree）
